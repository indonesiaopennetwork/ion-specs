# Hospitality F&B Delivery — Design Notes
...
| Question | Hospitality F&B | Trade F&B |
|---|---|---|
...
| CRC | `HSC-fnb-delivery` | `TRC-food-bev` |

## Customisation model

F&B delivery commonly uses COMPOSED resources with mandatory and optional customisation groups:
...
All F&B delivery resources MUST declare:
- `fnb.classification` — the halal/dietary status
- `fnb.allergens` — as an array (empty array is a valid declaration of "none")
...
## BPOM for hospitality F&B

Most restaurant-prepared food does not require individual BPOM registration. Packaged sauces, condiments, or beverages that the restaurant uses as ingredients may carry BPOM numbers — these are optional to declare via `fnb.bpomRegistration`.
...
- `fnb.isAlcoholic: true` is mandatory
- `fnb.classification` should be `NON_HALAL`
