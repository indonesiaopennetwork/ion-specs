...
## Shipment identity by pattern

| Pattern | Identity field | Issued at |
|---|---|---|
| parcel | `awbNumber` | `/on_confirm` |
| hyperlocal | `bookingReference` | `/on_confirm` |
| freight | `billOfLadingReference` | `/on_confirm` |
| roro | `ticketReference` | `/on_confirm` |
| warehouse | `storageContractReference` | `/on_confirm` |
...
**`/track` → `/on_track`** — real-time streaming handle. BAP requests a tracking URL or WebSocket endpoint. For hyperlocal, this streams live GPS coordinates. For parcel, it returns the AWB tracking URL that the carrier's website serves.

These are complementary: `on_status` for state machine events, `track/on_track` for continuous real-time position.

## Routing topology

For parcel, `routingTopology` is set in `/on_select` and confirmed in `/on_confirm`:
...
For hyperlocal, `agentDetails` carries: rider name, phone via proxy (never the real number), vehicle type, vehicle plate, and photo URL. When this is exposed depends on `offer.agentAssignmentModel`:
...
These fire in `on_status[PICKED_UP]` and `on_status[DELIVERED]` when cold chain is declared on the resource. If `coldChainIntegrityVerified = false` at delivery, the damage-claim variant is immediately eligible.
...
If actual weight exceeds declared by more than the `offer.weightDisputePolicy` tolerance, BPP raises the weight-dispute variant via `/on_update`.
...
After DELIVERY_ATTEMPT_FAILED, the delivery-attempts variant opens. BAP has until `buyerResolutionDeadline` to reschedule, change address, or switch to self-pickup.
...
Invalid `performanceState` transition → `ION-4xxx`. Missing `awbNumber` at PICKED_UP → `ION-4xxx`. See `errors/README.md` for the full error code reference.
