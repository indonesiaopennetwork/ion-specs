...
| payment-COD-BAP-collected | init → on_status[DELIVERED] | COD where marketplace collects. COD amount is declared in `considerationAttributes.codAmount`. |
...
| on-network-LSP | on_confirm | BPP triggers a separate on-network Logistics Contract after trade confirm. The Logistics Contract uses `parentContractReference`. |
