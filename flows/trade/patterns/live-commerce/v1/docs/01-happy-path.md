...
3. **Select with attribution.** BAP sends `/select` including inherited `source` with channel, creator, content, affiliate, or group-buy identifiers as applicable.
...
For OTT-embedded flow specifically: the consumer may tap a product during playback, or after playback completes (if the OTT platform preserves the shoppable moment). `source.contentTimestampSeconds` captures the original in-content moment regardless of when the tap happens.
